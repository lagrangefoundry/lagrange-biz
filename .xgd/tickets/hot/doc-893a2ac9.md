...
id: DOC-4
type: doc
title: How Can You Trust the Code Your AI Writes?
created_by: xgd
created_at: '2026-06-28T21:42:57.734252+00:00'
updated_at: '2026-07-25T22:27:17.987080+00:00'
completed_at: null
...
# How Can You Trust the Code Your AI Writes?
...
In 2025, a new mode of development went viral: describe intent, accept AI-generated changes without question, ship at extraordinary speed. It acquired a name: _vibe coding_ [1]. Within a year its creator, Andrej Karpathy, had moved on, declaring it a _transitional phase_. Its successor, which he termed _agentic engineering_ [2], is a professional discipline: coordinating AI agents while preserving correctness, quality, and taste — designing specs, supervising plans, writing tests, building evaluation loops. Vibe coding raised the floor; agentic engineering raises the ceiling. The shift was really driven by something that all practitioners see when using AI coding: the _scaling wall_ — the point at which AI-assisted development, previously exhilarating in its speed, starts running aground because new changes create behavioral regressions that cannot be managed.

We use _generative development_ to describe any software development process in which AI systems produce the majority of the code — the regime that current tools have already reached for many practitioners. Vibe coding, agentic engineering, and governed generative development are all variants. This paper's question is not whether to develop this way, but how to do so while retaining the behavioral integrity of the project.

The scaling wall is not a failure of AI capability. At the task level, productivity gains are real and sustained: the AI keeps writing code, and the code largely works. The problem is cumulative regressions. Each change to a growing codebase carries risk — something that worked before may break — and without reliable mechanisms to detect regressions, behavioral integrity silently erodes. Forward progress eventually stalls altogether — the AI keeps producing code, but the changes it introduces carry unanticipated consequences that compound. We argue that this problem is structural — and must therefore be solved by process infrastructure, not model capability.

This paper identifies the forces that lead to the scaling wall, distinguishes those that better models will eventually resolve from those they cannot, and describes the structural requirement that remains: an explicit, maintained record of what software is supposed to do — its _behavioral identity_ — that makes regression detection reliable and system behavior auditable. We introduce the Software Artificer as the emerging technical human role that governs AI-driven development at scale. A companion paper describes XGD, a platform built to automate these ideas, and reports on nine months of research: using it to build itself.
...
2. The Scalability Wall is Real
...
Getting behavioral identity out of people's heads and into explicit artifacts changes the development equation. The same AI models that write code can build and maintain a behavioral specification — keeping it current as intents are applied, and using it to resolve the question a failing test cannot answer from code alone. Once behavioral intent is captured this way, the development workflow becomes automatable in ways that human discipline cannot achieve. The team is freed from reconstructing intent to focus on higher-level questions of specification and direction. But this payoff depends on the artifact being well-formed — an imprecise specification, like an imprecise test, answers the wrong question.

There is a further subtlety: behavioral identity is accumulated current state, built from every design decision applied since inception — every feature, every bug resolved, every scope change. The analogy is a database schema: the schema reflects every migration ever applied, but it is not a migration log — it is the current truth. A living behavioral specification works the same way, recording the accumulated result of all decisions rather than the history of how they were reached.
...
A governed generative development framework must therefore maintain behavioral identity as a living artifact: updated automatically as new intents are applied, reflecting the current accumulated state of intended behavior. This is what lets an autonomous system resolve a failing test without human input.

Behavioral identity has a further property that becomes apparent only when the specification is externalized: it turns the auditability problem raised above into something solvable. An externalized behavioral specification — human-readable, independent of any model session, open to inspection by anyone — is what makes a system's behavior demonstrable rather than merely believed. In AI-driven development, where no human has reviewed every line of code, this accountability layer is essential.

### 5. The Software Artificer: A Role for the New Development Paradigm

This shift in how software is produced implies a corresponding shift in the human role. We propose a title for this role: the Software Artificer (SA) — borrowed from the medieval craftsperson who directs a skilled workshop rather than wielding every tool personally. This is neither a "programmer assisted by AI support" nor a product manager.
...
The framework described here — behavioral identity as a living, operationally-verified artifact, governed by automated process infrastructure, overseen by a Software Artificer — is already proven in practice. A companion paper describes nine months of building a production platform using these principles, and reports what the data shows.
